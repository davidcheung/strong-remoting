...
function toXML(input, method) {
  var xml;
  var returns = method.returns[0];
  var wrapperElement;
  if (returns.xml && typeof(returns.xml.wrapperElement) == 'string') {
    wrapperElement = returns.xml.wrapperElement;
  } else {
    wrapperElement = 'response';
  }
  if (input && typeof input.toXML === 'function') {
    ...
    xml = js2xmlparser(wrapperElement, input, {
      prettyPrinting: {
        indentString: '  '
      },
      convertMap: {
  ...
  if (returns.xml && returns.xml.wrapperElement === false) {
    //allowing XML output to have no root element
    var match = xml.match(/<response>([\s\S]*?)<\/response>$/);
    xml = match[1].replace(/^[\s]{0,2}/gim, '').trim();
  }
  return xml;
}
